...
func _on_EmployeeTree_button_pressed(item, column, id):
	...
	if new_status == "in":
		item.set_button(column, id, load("res://art/office.svg"))
	elif new_status == "out":
...
func load_person_into_tree(current_person):
	...
	if current_person.Status == "in":
		current_tree_item.add_button(4, load("res://art/office.svg"))
	elif current_person.Status == "out":
		current_tree_item.add_button(4, load("res://art/house.svg"))
